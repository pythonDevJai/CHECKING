...
@app.route('/get-multiple-record-from-studentinfo' , methods = ['GET'])
def getMultipleRecords():
    request_name = request.args.get('name')
    request_age = request.args.get('age')
    records = session.query(StudentInfo).filter(or_(StudentInfo.Name == request_name , StudentInfo.Age == request_age)).all()
