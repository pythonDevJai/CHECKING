...
@app.route('/get-Single-record-from-studentinfo' , methods = ['GET'])
def getSingleRecordss():
    request_name = request.args.get('name')
    request_age = request.args.get('age')
    records = session.query(StudentInfo).filter(and_(StudentInfo.Name == request_name , StudentInfo.Age == request_age)).all()
    converting_to_dict = [item.__dict__ for item in records]
    for item in converting_to_dict:
        del item['_sa_instance_state']

    return json.dumps(converting_to_dict)
